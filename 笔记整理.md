...
在java中根据一个子类获取其父类或接口信息非常方便，但是根据一个接口获取该接口的所有实现类却没那么容易。 这个时候就可以使用ServiceLoader来实现上述需求。java本身提供了一种方式来获取一个接口的子类，那就是使用java.util.ServiceLoader#load(java.lang.Class<S>)
