...
![web-structure](./image/2021/web.jpg)

官网大致架构如图所示。前后台系统使用前后端分离模式，因此是4个工程。各个工程采用docker 集群部署。

【管理后台系统】前后端工程通过内部网关（inner-gateway）进行交互；
【前台系统】前后端工程通过外部网关（out-gateway）进行交互。

所有系统均部署在aws云服务器上。

## 第一版本官网

第一版本的官网，图片等资源使用aws的cdn缓存，网站document数据开启gzip配置，访问页面基本需要2-3秒，图片等资源加载则更慢。因为即便是配置了CDN，由于CDN节点有多个，且aws的CDN节点离国内最近的也在东京，因此速度会受到一定的限制。

## 第二版本官网

第二版相对于第一版，主要解决了图片等资源的加载慢的问题。此次启用了智能cdn解析，根据浏览器入口所在地区判断是要访问oss还是s3。

相应的资源文件要在oss和s3都存储一份，且需要保证文件的directory一致。

这次优化以后，图片等资源加载速度确实变快，基本在200ms内，但是首屏的html文件加载还是很慢，因为这个文件的数据部分需要从后端服务获取，要走out-gateway。

## 第三版本官网

第三版本相对于第二版本，主要优化了网关配置。由于之前网关为了方便国内用户访问，设置了加速url，ip在国内。

官网网络的数据流向相当于从aws==》ali，然后ali==》aws，因此慢。后续改成使用网关的集群内部ip地址，直接aws==》aws访问。
