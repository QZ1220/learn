...
```
从源码可以看出，get操作确实没有加锁。并且，table使用了volatile关键字进行修饰，依据JVM到happen-before原则，保证了数据到内存可见性，也就是保证了在并发读写时，读取到到数据是最新到。
...
首先是根据put的key到hash定位需要放置到位置，如果当前位置没有严肃，那么使用cas放置元素。
...
ngx_http_limit_req_module配置就要稍微复杂一些。
