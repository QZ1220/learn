...
在JDK 8中，Java的String底层的实现发生了一些重要的改变。之前，String类的底层实现采用的是基于字符数组的方式（char[]），但在JDK 8中引入了一种新的实现方式，即基于字节数组的方式（byte[]）。

这个改变主要是为了解决字符串存储和处理中的一些性能和内存占用问题。下面是改变的一些主要原因：

 1. 空间效率：基于字节数组的实现可以节省内存空间，特别是在存储ASCII字符（占用一个字节）的时候。之前的基于字符数组的实现需要使用2个字节来存储一个ASCII字符，造成了内存的浪费。
 2. 编码支持：基于字节数组的实现可以更好地支持不同的字符编码，特别是针对Unicode字符集的全面支持。它可以灵活地存储不同长度的字节序列，以支持各种不同的字符编码，如UTF-8、UTF-16等。
 3. 字符串拼接效率：在字符串拼接操作中，基于字节数组的实现可以更高效地处理，而不再需要像基于字符数组的实现那样，每次拼接都创建一个新的字符数组。基于字节数组的实现可以使用可变长度的字节数组，避免了频繁创建和复制字符数组的开销。

需要注意的是，这个改变并不意味着所有的字符串都是基于字节数组的实现方式。JDK 8引入了String的coder字段，用于标识字符串的底层实现方式，它可能是基于字节数组的实现（coder=）或者基于字符数组的实现（coder=1）。根据具体的字符串内容和操作，JVM会根据需要选择最适合的底层实现方式。

总的来说，JDK 8中String底层实现的改变主要是为了提高空间效率、提供更好的编码支持，并优化字符串的拼接操作。这些改变在一定程度上提升了字符串的性能和效率，同时也更好地适应了多样化的字符编码需求。
...
◇ Using where with pushed condition：通俗来讲就是`索引下推`，这是一个仅仅在InnoDB 存储引擎中才会出现的信息，而且还需要通过打开Condition Pushdown 优化功能才可能会被使用。控制参数为engine_condition_pushdown 。
...
## GC回收算法
JVM将堆分成了 二个大区  Young 和 Old ，二者默认的内存占比为1：2。

 - Young区

Young 区又分为 Eden、Survivor1、Survivor2,默认占比为8：1：1， 两个Survivor 区相对地作为为From 和 To 逻辑区域, 当Servivor1作为 From 时 ， Servivor2 就作为 To,反之亦然。这是因为新生代区域使用的是**复制清理**的GC算法。
...
```
### 除了JDK动态代理和CGLIB动态代理两种AOP方案，还有什么方法可以实现AOP

1. AspectJ：AspectJ是一个功能强大的AOP框架，它采用编译时增强技术，通过在编译阶段对代码进行织入，实现对横切关注点的管理。AspectJ提供了更丰富的切点表达式和语法，可以实现更细粒度的AOP。

2. Spring AOP：Spring AOP是Spring框架提供的AOP支持。它基于动态代理技术，通过拦截器链和切点表达式实现对方法的增强。Spring AOP提供了声明式的方式来定义切面，并与Spring IoC容器紧密集成，可以很方地在Spring应用中使用AOP。

3. Javassist：Javassist是一个基于字节码操作的Java代码生成工具库，它提供了编程方式动态修改字节码的能力。通过Javassist，可以在运行时动态修改类的字节码，实现对类的增强，从而实现AOP的效果。

4. Byte Buddy：Byte Buddy是一个轻量级的Java字节码生成和操作库，它提供了简单的API和流式操作符，用于动态生成和修改Java类的字节码。通过Byte Buddy，可以在运行时动态创建类、扩展类，实现对类的增强和AOP功能。
...
```
核心线程数2，最大线程数5，如果最大线程数大于核心线程数，那么在其空闲2秒以后将会被回收，直到剩下核心线程。需要注意的是，核心线程也是可以被回收的，通过`ThreadPoolExecutor`的`allowCoreThreadTimeOut`方法进行配置。

使用如下测试代码，查看线程池启动、运行、后续的一整个过程：
```java
    public static void main(String[] args) {
...
JDK线程池的线程复用算法是基于工作窃取（Work-Stealing）的。JDK线程池中的线程池工作线程采用了一种称为"ForkJoinPool"的线程池实现。

在ForkJoinPool中，每个工作线程都有一个自己的工作队列（Work Queue），工作队列采用了一种双端队列（Deque）的数据结构。当一个线程需要执行任务时，它会将任务提交到自己的工作队列中。当一个线程完成了当前任务后，会从自己的工作队列中取出下一个任务进行执行。

这里涉及到两种情况：

- 当一个线程的工作队列为空时，它可以从其他线程的工作队列中"偷"取任务。偷取的任务通常是队列的尾部（TAIL任务，这样可以保证线程之间的工作均衡，避免某些线程一直处于繁忙状态。
- 当一个线程的工作队列中只剩下一个任务时，它可能会主动与其他线程合并（Join）工作队列，这样可以减少线程的数量，节省系统资源。合工作队列是一个递归的过程，将多个线程的工作队列合并为一个更大的队列，从而减少线程的数量。
通过这种工作窃取算法，JDK线程池能够充分利用线程的并行能力，提高处理任务的效率。工作窃取算法在处理大量独立任务的情况下尤为高效，因为任务之间不存在赖关系，可以自由地进行工作窃取和并行执行这样可以使线程池尽可能地保持高效的利用率，提高应用程序的性能。
...
#### CountDownLatch的await方法底层实现原理

在CountDownLatch内部，维护着一个初始值为N的计数器，该计数器通过构造函数传入。每次调用CountDownLatch的countDown方法时，计数器的值减1。当计数器的值变为零时，所有调用await方法而阻塞的线程将被唤醒，可以继续执行。

await方法的实现原理如下：

1. 在调用await时，当前线程会尝试获取CountDownLatch内部的共享锁。
2. 如果计数器的值大于0，表示还未达到条件，当前线程将被阻塞。
3. 如果计数器的值已经变为0，表示已经满足条件，当前线程不需要阻塞，直接返回。
4. 当其他线程调用countDown方法，减少计数器的值，直到计数器的值变为0时，会释放共享锁，并唤醒所有处于阻塞状态的线程。
5. 被唤醒的线程会再次检查计数器的值，如果满足条件，则继续执行，否则重新进入阻塞状态。

CountDownLatch的底层实现依赖于原子操作和线程同步机制，确保线程间的可见性和正确的阻塞和唤醒行为。在调用await方法时，当前线程会进入等待状态，直到计数器的值变为零后，被唤醒并继续执行后续的逻辑。这种方式可以很好地实现多线程间的协调和同步。
