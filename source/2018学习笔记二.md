...
* [2018学习笔记二](#2018学习笔记二)
   ...
   * [http握手挥手](#http握手挥手)
      ...
      * [四次挥手](#四次挥手)
         * [为什么是四次挥手](#为什么是四次挥手)
         * [为什么需要TIME_WAIT状态](#为什么需要time_wait状态)
         * [服务器为什么会出现大量CLOSE_WAIT状态](#服务器为什么会出现大量close_wait状态)
   * [TCP滑动窗口](#tcp滑动窗口)
...
ETag是一个可以与Web资源关联的记号（token）。典型的Web资源可以一个Web页，但也可能是JSON或XML或者图片等资源类文件。
...
redis的数据结构还可以参考另外一篇[笔记](https://github.com/AudiVehicle/learn/blob/master/source/redis%E5%AD%A6%E4%B9%A0.md#redis%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84)。
...
注意IO多路复用和NIO不是一个东西，NIO仅仅是指IO API总是能立刻返回，不会被Blocking；而IO多路复用仅仅是操作系统提供的一种便利的通知机制。

当然二者要结合一起使用，才能避免程序因为等待IO而被卡住。

- https://www.cnblogs.com/twoheads/p/10712094.html
- https://mp.weixin.qq.com/s?__biz=Mzk0MjE3NDE0Ng==&mid=2247494866&idx=1&sn=0ebeb60dbc1fd7f9473943df7ce5fd95&chksm=c2c5967ff5b21f69030636334f6a5a7dc52c0f4de9b668f7bac15b2c1a2660ae533dd9878c7c&scene=21#wechat_redirect

要实现IO多路复用，可以借助操作系统的如下三个系统调用函数：

#### select
我们可以把一个文件描述符的数组发给操作系统， 让操作系统去遍历，确定哪个文件描述符可以读写，然后告诉我们去处理：

![select](./image/2018/select.gif)

如上面动图所示，我们一次性给内核提交了一个fd（文件描述符）数组，有一种的批量的思想在里面，避免了单个单个的系统调用。

但是对于内核给应用程序返回的fd数组，程序依然需要遍历这个fd数组，才能知道哪些数据是就绪的。
...
整个select函数的调用的过程如下图所示：

![select](./image/2018/select.jpg)

从图上也可以看出一次select调用完成后，会伴随多次的read调用。

#### poll

poll大体上和select是类似的，只是取消了select的fd数组的1024的大小限制。
...
4. 203 Non-Authoritative Information（自HTTP / 1.1起）
...
9. 308 Permanent Redirect (RFC 7538)
...
2. 501 Not Implemented（未实现）：服务器不支持当前请求所需要的功能或无法完成请求。

3. 502 Bad Gateway（错误的网关）：服务器作为网关或代理，从上游服务器收到无效的响应。

4. 503 Service Unavailable（服务不可用）：服务器目前无法处理请求，一般是由于过载或停机维护。

5. 504 Gateway Timeout（网关超时）：服务器作为网关或代理，未及时从上游服务器接收到响应。

6. 505 HTTP Version Not Supported（HTTP版本不支持）：服务器不支持请求中所使用的HTTP协议版本。
