...
## 半同步复制
mysql的半同步复制，主要是为了解决异步复制时数据一致性问题以及同步复制时性能问题。

所谓半同步复制，就是指在master执行事务，未commit前（在commit前还是后可以进行配置），需要等待至少一个slave同步当前数据成功，或者超时以后使用异步复制模式。
